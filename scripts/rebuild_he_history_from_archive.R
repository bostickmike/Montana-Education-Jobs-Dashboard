...
# from the Wyoming Education Jobs Dashboard's own
# scripts/rebuild_he_history_from_archive.R.
#
...
if (sys.nframe() == 0) {
  ...
  write.csv(result$facultydata, file.path("Mt_Ed_Jobs", "facultydata.csv"), row.names = FALSE)
  write.csv(result$allsum_he, file.path("Mt_Ed_Jobs", "allsum_he.csv"), row.names = FALSE)
  write.csv(result$he_institution_weekly_totals, file.path("Mt_Ed_Jobs", "he_institution_weekly_totals.csv"), row.names = FALSE)
