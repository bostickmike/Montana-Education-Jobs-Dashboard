...
# (AppliTrack, SchoolSpring): reads the district registry, scrapes each
# district live via safe_scrape(), classifies the results, and prints a
# summary. Not the final pipeline entry point (that will be Mt_ED_Jobs.Rmd,
# mirroring Wyoming's Wy_ED_Jobs.Rmd) -- this is a standalone check that
# every ported scraper actually works against real Montana job boards
# before the full pipeline gets built on top of them.
#
# AppliTrack and SchoolSpring return different column name casing
# (title/position/location vs. Title/Location/Link) -- the same
...
cat("Total K-12 postings found:", nrow(applitrack_combined) + nrow(schoolspring_combined), "\n\n")
