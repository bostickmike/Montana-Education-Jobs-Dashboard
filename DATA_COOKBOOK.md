...
One current row per this project's **128 directly-scraped K-12 districts** (`k12_district_registry.csv`) — not Montana's full ~398-district universe, matching this project's job-postings scraping scope. `District`/`County`/`Latitude`/`Longitude`/`Job_Link` come from the registry; everything else is refreshed from its live source every run (and left untouched if that run's fetch fails, rather than being blanked out). `MT_CCD_LEA_MAP`/`MT_OPI_FINANCE_LEA_MAP`/`MT_SAIPE_DISTRICT_MAP` (staffing, finance, child poverty) cover 32 of the 128 registered districts — Browning, Libby (added 2026-08-11), Broadview, Conrad, Westby, Choteau, Gardiner, Malta, Drummond, Custer, Scobey, Deer Lodge, Townsend, Three Forks, Anaconda, and Philipsburg (added 2026-08-16), and Corvallis, Cascade, Hays-Lodge Pole, Plevna, Sunburst, Ramsay, Roy, Arrowhead, North Star, Belt, Big Sky, Melstone, Shields Valley, Manhattan, Roundup, White Sulphur Springs, Shelby, Geyser, Centerville, Thompson Falls, Arlee, Chinook, Darby, Dutton/Brady, and Ekalaka (added 2026-08-23), plus Trego, St. Ignatius, Stanford, Lolo, Reed Point, Hobson, Froid, Terry, Gallatin Gateway, Turner, Swan River, Bigfork, Lincoln, Independent, Shepherd, Hysham, Huntley Project, Park City, Fort Benton, Glasgow, Alberton, Dillon, Baker, Cut Bank, Bridger, Red Lodge, Joliet, Ennis, Columbus, Hinsdale, St Regis, Valier, Roberts, Winnett, Florence-Carlton, Miles City, West Yellowstone, Augusta, Brockton, Culbertson, Cottonwood (added 2026-08-24), Bonner, Creston, Clinton, Deer Park, Dixon, Elysian, Evergreen, Fair-Mont-Egan, Lone Rock, Lustre, Miami, Target Range, Zurich, and Dayton (added 2026-08-25), all via a targeted OPI-gap platform/heuristic check, are a fast-follow gap not yet closed, the same pattern Wolf Point and Plentywood went through in 2026-08-07 before their own gap closed same-day. **One real, permanent exception remains**: `MT_DLI_DISTRICT_MAP` (teacher salary) covers only 27 of the 128 registered districts — Helena, Lockwood, and Glendive are genuinely "ND" (non-disclosable) in their own regional PDF row despite a real disclosed teacher count, and Lame Deer and Lodge Grass don't appear as a row in any of MT DLI's 9 regional PDFs at all. All five show real `NA` for `Teacher_Avg_Salary` and related columns — confirmed live, not an unfinished fast-follow. (The 96 fast-follow districts above add 96 more real `NA`s on top of those five, for the separate fast-follow reason above.)
...
| `Data_Coverage` | text | `k12_district_registry.csv`'s own `Platform` column, via `k12_district_coverage_tiers()` in `misc_district_scrapers.R` | Added 2026-09-04, ported from Wyoming's own `Data_Coverage`/`misc_district_coverage_tiers()` disclosure feature. `"Full"` for a district scraped via a real, documented ATS API (`AppliTrack`/`SchoolSpring`/`TedK12`/`TylerPortico` — `direct_api_scrapers.R`); `"Partial"` for every other directly-scraped district (`Apptegy`/`RedRoverK12`/every single-district `*Heuristic` platform — `misc_district_scrapers.R`'s own header comment already calls this whole file "a real but acknowledged-less-reliable source"). Unlike Wyoming, this is a single tier, not two — Montana has one unified registry (no separate misc-district registry) and OPI-direct dedup already applies uniformly to every registered district, not just a heuristic subset, so there's no Wyoming-style "WSBA + own page" vs. "WSBA only" split to make here. Shown as a red badge on Map popups and a "Posting Data" column in the K-12 Summary table (`app.R`) — currently 41 `"Full"` / 87 `"Partial"` of the 128 registered districts. HE institutions in `salarymap.csv`/`map_he` carry `Data_Coverage = NA` instead of a hardcoded `"Full"` (Wyoming's choice) — Montana has real heuristic HE scrapers too (`misc_college_scrapers.R`, ~10 of 23 institutions), so hardcoding `"Full"` there would misrepresent them; no HE tier has been designed yet. |
...
One row per open posting across this project's 23 registered HE institutions, every job type (not just faculty). Rebuilt fresh every run — **not** accumulated (the accumulated raw history lives in `Archived_HE_Data/*.xlsx`, one file per run).
...
One current row per this project's **23 registered HE institutions** — matching Wyoming's own `salarymap.csv` scope (its 9 rows are exactly its 9 job-scraped institutions, not a broader MUS-wide reference set), not the full ~16-institution MUS-plus-tribal-college universe `RESEARCH_NOTES.md` originally scoped for job-postings coverage (a coincidental near-match in count, not the same set of institutions). Of the 23, 4 (Highlands College, Helena College, University of Montana Western, and Missoula College) aren't independently scraped — they ride a parent institution's shared job board (Montana Tech's JazzHR board, University of Montana's NEOGOV board) and are attributed per-posting to their own real institution instead of their parent's count (`Platform` is `"SharesBoard"` for these 4, not the platform they actually ride). `Name`/`County`/`Longitude`/`Latitude`/`Link` come from `he_institution_registry.csv`; everything else refreshes from IPEDS/FSA each run. `MT_IPEDS_UNITID_MAP` (`ipeds_salary_scraper.R`) covers 22 of the 23 — extended 2026-08-07 from the original 6 to cover 7 more, then the 4 tribal colleges, then Highlands College/Helena College/UM Western once their postings were split out from their parent institutions' feeds, each confirmed live against both the Urban Institute's college directory and its own real salary/enrollment/Pell response. **Missoula College is the one real, permanent exception**: confirmed live it has no independent unitid in the Urban Institute's IPEDS directory for Montana at all (not even a branch ID) — its salary/enrollment/Pell figures are fully consolidated into University of Montana's own reporting, the same structural case as Bitterroot College/Gallatin College MSU noted in `RESEARCH_NOTES.md`. Highlands College's own case is a *partial* gap, not a full one: confirmed live it has no salary or Pell data under its own unitid (likely consolidated into Montana Tech's HR/financial-aid reporting), but real, separate fall-enrollment data.
...
- **Montana's job-postings scraping scope (128 K-12 districts, 23 HE institutions as of 2026-08-25) is intentionally narrower than Montana's full universe** (~398 nominal K-12 districts, ~16 MUS+tribal HE institutions per `RESEARCH_NOTES.md`) — every registry-keyed dataset above (`salarymap2.csv`, `salarymap.csv`) matches that same narrower scope, not the full state. The OPI statewide feed is the one exception that surfaces postings (not salary/staffing data) from districts outside the registry, with the caveats noted above. Within that 128/23 scope, `MT_CCD_LEA_MAP`/`MT_OPI_FINANCE_LEA_MAP` cover 32 of the 128 K-12 districts (Browning/Libby/Broadview/Conrad/Westby/Choteau/Gardiner/Malta/Drummond/Custer/Scobey/Deer Lodge/Townsend/Three Forks/Anaconda/Philipsburg/Corvallis/Cascade/Hays-Lodge Pole/Plevna/Sunburst/Ramsay/Roy/Arrowhead/North Star/Belt/Big Sky/Melstone/Shields Valley/Manhattan/Roundup/White Sulphur Springs/Shelby/Geyser/Centerville/Thompson Falls/Arlee/Chinook/Darby/Dutton-Brady/Ekalaka/Trego/St. Ignatius/Stanford/Lolo/Reed Point/Hobson/Froid/Terry/Gallatin Gateway/Turner/Swan River/Bigfork/Lincoln/Independent/Shepherd/Hysham/Huntley Project/Park City/Fort Benton/Glasgow/Alberton/Dillon/Baker/Cut Bank/Bridger/Red Lodge/Joliet/Ennis/Columbus/Hinsdale/St Regis/Valier/Roberts/Winnett/Florence-Carlton/Miles City/West Yellowstone/Augusta/Brockton/Culbertson/Cottonwood/Bonner/Creston/Clinton/Deer Park/Dixon/Elysian/Evergreen/Fair-Mont-Egan/Lone Rock/Lustre/Miami/Target Range/Zurich/Dayton are a fast-follow gap, not yet closed — see `salarymap2.csv`'s section above) (`MT_DLI_DISTRICT_MAP` covers 27, five of those a real permanent gap plus that same 96-district fast-follow gap on top), and `MT_IPEDS_UNITID_MAP` covers 22 of the 23 HE institutions (Missoula College is a real permanent gap — see `salarymap.csv`'s section above) — not the same thing as this overall scope note.
