...
One current row per this project's **79 directly-scraped K-12 districts** (`k12_district_registry.csv`) — not Montana's full ~398-district universe, matching this project's job-postings scraping scope. `District`/`County`/`Latitude`/`Longitude`/`Job_Link` come from the registry; everything else is refreshed from its live source every run (and left untouched if that run's fetch fails, rather than being blanked out). `MT_CCD_LEA_MAP`/`MT_OPI_FINANCE_LEA_MAP`/`MT_SAIPE_DISTRICT_MAP` (staffing, finance, child poverty) cover 32 of the 79 registered districts — Browning, Libby (added 2026-08-11), Broadview, Conrad, Westby, Choteau, Gardiner, Malta, Drummond, Custer, Scobey, Deer Lodge, Townsend, Three Forks, Anaconda, and Philipsburg (added 2026-08-16), and Corvallis, Cascade, Hays-Lodge Pole, Plevna, Sunburst, Ramsay, Roy, Arrowhead, North Star, Belt, Big Sky, Melstone, Shields Valley, Manhattan, Roundup, White Sulphur Springs, Shelby, Geyser, Centerville, Thompson Falls, Arlee, Chinook, Darby, Dutton/Brady, and Ekalaka (added 2026-08-23), plus Trego, St. Ignatius, Stanford, Lolo, Reed Point, and Hobson (added 2026-08-24), all via a targeted OPI-gap platform/heuristic check, are a fast-follow gap not yet closed, the same pattern Wolf Point and Plentywood went through in 2026-08-07 before their own gap closed same-day. **One real, permanent exception remains**: `MT_DLI_DISTRICT_MAP` (teacher salary) covers only 27 of the 79 registered districts — Helena, Lockwood, and Glendive are genuinely "ND" (non-disclosable) in their own regional PDF row despite a real disclosed teacher count, and Lame Deer and Lodge Grass don't appear as a row in any of MT DLI's 9 regional PDFs at all. All five show real `NA` for `Teacher_Avg_Salary` and related columns — confirmed live, not an unfinished fast-follow. (The 47 fast-follow districts above add 47 more real `NA`s on top of those five, for the separate fast-follow reason above.)
