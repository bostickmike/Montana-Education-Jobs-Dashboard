...
One current row per this project's **30 directly-scraped K-12 districts** (`k12_district_registry.csv`) — not Montana's full ~398-district universe, matching this project's job-postings scraping scope. `District`/`County`/`Latitude`/`Longitude`/`Job_Link` come from the registry; everything else is refreshed from its live source every run (and left untouched if that run's fetch fails, rather than being blanked out). **Note a real coverage gap**: `MT_DLI_DISTRICT_MAP`/`MT_CCD_LEA_MAP`/`MT_SAIPE_DISTRICT_MAP` (salary, staffing, child poverty) still only cover the original 18 districts registered before the 2026-08-07 K-12 expansion — the 12 added since (Glendive, Colstrip, Sidney, Stevensville, Big Sandy, Lame Deer, Poplar, Fairview, Rocky Boy, Ronan, Livingston, Lodge Grass) show real `NA` for those columns, a deliberate fast-follow not yet done. `MT_OPI_FINANCE_LEA_MAP` (below) is the one map already extended to all 30.
...
One current row per this project's **13 directly-scraped HE institutions** — matching Wyoming's own `salarymap.csv` scope (its 9 rows are exactly its 9 job-scraped institutions, not a broader MUS-wide reference set), not the full ~16-institution MUS-plus-tribal-college universe `RESEARCH_NOTES.md` originally scoped for job-postings coverage. `Name`/`County`/`Longitude`/`Latitude`/`Link` come from `he_institution_registry.csv`; everything else refreshes from IPEDS/FSA each run. `MT_IPEDS_UNITID_MAP` (`ipeds_salary_scraper.R`) was extended 2026-08-07 from the original 6 institutions to cover all 13 currently registered — each new unitid confirmed live against both the Urban Institute's college directory and its own real salary/enrollment/Pell response, not just matched by name.
