...
One current row per this project's **42 directly-scraped K-12 districts** (`k12_district_registry.csv`) — not Montana's full ~398-district universe, matching this project's job-postings scraping scope. `District`/`County`/`Latitude`/`Longitude`/`Job_Link` come from the registry; everything else is refreshed from its live source every run (and left untouched if that run's fetch fails, rather than being blanked out). `MT_CCD_LEA_MAP`/`MT_OPI_FINANCE_LEA_MAP`/`MT_SAIPE_DISTRICT_MAP` (staffing, finance, child poverty) cover 32 of the 42 registered districts — Browning, Libby (added 2026-08-11), Broadview, Conrad, Westby, Choteau, Gardiner, Malta, Drummond, and Custer (added 2026-08-16), all via a targeted OPI-gap platform/heuristic check, are a fast-follow gap not yet closed, the same pattern Wolf Point and Plentywood went through in 2026-08-07 before their own gap closed same-day. **One real, permanent exception remains**: `MT_DLI_DISTRICT_MAP` (teacher salary) covers only 27 of the 42 registered districts — Helena, Lockwood, and Glendive are genuinely "ND" (non-disclosable) in their own regional PDF row despite a real disclosed teacher count, and Lame Deer and Lodge Grass don't appear as a row in any of MT DLI's 9 regional PDFs at all. All five show real `NA` for `Teacher_Avg_Salary` and related columns — confirmed live, not an unfinished fast-follow. (The 10 fast-follow districts above add 10 more real `NA`s on top of those five, for the separate fast-follow reason above.)
...
- **Montana's job-postings scraping scope (42 K-12 districts, 23 HE institutions as of 2026-08-16) is intentionally narrower than Montana's full universe** (~398 nominal K-12 districts, ~16 MUS+tribal HE institutions per `RESEARCH_NOTES.md`) — every registry-keyed dataset above (`salarymap2.csv`, `salarymap.csv`) matches that same narrower scope, not the full state. The OPI statewide feed is the one exception that surfaces postings (not salary/staffing data) from districts outside the registry, with the caveats noted above. Within that 42/23 scope, `MT_CCD_LEA_MAP`/`MT_OPI_FINANCE_LEA_MAP` cover 32 of the 42 K-12 districts (Browning/Libby/Broadview/Conrad/Westby/Choteau/Gardiner/Malta/Drummond/Custer are a fast-follow gap, not yet closed — see `salarymap2.csv`'s section above) (`MT_DLI_DISTRICT_MAP` covers 27, five of those a real permanent gap plus that same 10-district fast-follow gap on top), and `MT_IPEDS_UNITID_MAP` covers 22 of the 23 HE institutions (Missoula College is a real permanent gap — see `salarymap.csv`'s section above) — not the same thing as this overall scope note.
