# Tier-1 boot smoke -- modelled on the LASSO project's tests/smoke.R.
#
...
# no JavaScript, no htmlwidgets. This file boots the REAL app.R in a headless
# browser against the real committed CSVs and checks it renders end to end:
# every tab, no Shiny error banner anywhere, and the leaflet / plotly / DT
# widgets actually came back.
#
...
#
# Needs shinytest2 (pulls chromote / headless Chrome). ~1-2 min.
...
skip_if_not(dir.exists(app_dir), "Mt_Ed_Jobs/ not found")
skip_if_not(file.exists(file.path(app_dir, "combinedclean.csv")),
            "committed CSVs not found -- run the pipeline first")

# every leaf tab a user can actually land on (the two menuItem parents,
# k12_root / he_root, are containers, not destinations)
LEAF_TABS <- c(
  "intro", "map_tab",
...
# One AppDriver for the whole file (booting shinydashboard + leaflet + plotly
# + DT takes ~15s; don't pay it per test).
app <- NULL
...
test_that("app.R boots against the committed data without an error", {
  ...
  # htmlwidgets/leaflet sometimes log benign warnings; only fail on hard errors
...
test_that("clicking 'View all jobs' from a map popup jumps to the filtered Jobs Table", {
  skip_if(is.null(app), "app failed to boot")
